// Dafny program Lambda.dfy compiled into C#
...
// Command-line arguments: run --no-verify /home/under/Licenta/Library/Circuit-SAT/Lambda.dfy
// Lambda.dfy
  ...
  decreases t
{
  ...
  case Application(t1, t2) =>
    reunion(bound(t1), bound(t2))
}

function addition(x: Id, s: seq<Id>): seq<Id>
  requires allUnique(s)
  ensures var s': seq<Id> := addition(x, s); allUnique(s') && x in s'
  decreases x, s
{
  if x in s then
    s
  else
    assert !(x in s); assert x in s + [x]; assert allUnique(s + [x]); s + [x]
}

function reunion(s1: seq<Id>, s2: seq<Id>): seq<Id>
  requires allUnique(s1)
  requires allUnique(s2)
  ensures var s: seq<Id> := reunion(s1, s2); allUnique(s) && (forall str: Id {:trigger str in s2} {:trigger str in s1} {:trigger str in s} :: str in s ==> str in s1 || str in s2) && (forall str: Id {:trigger str in s} {:trigger str in s2} {:trigger str in s1} :: str in s1 || str in s2 ==> str in s) && |s| >= |s1| && |s| >= |s2|
  decreases s1, s2
{
  if s1 == [] then
    s2
...
ghost predicate allUnique(s: seq<Id>)
  decreases s
{
  forall id1: nat, id2: nat {:trigger s[id2], s[id1]} :: 
    id1 < id2 < |s| ==>
      s[id1] != s[id2]
}

lemma /*{:_inductionTrigger reunion(s1, s2)}*/ /*{:_induction s1, s2}*/ reunionIsGoodForWork(s1: seq<Id>, s2: seq<Id>)
  requires allUnique(s1)
  requires allUnique(s2)
  ensures allUnique(reunion(s1, s2))
  decreases s1, s2
{
  ghost var s := reunion(s1, s2);
  assert forall str: Id {:trigger str in s} {:trigger str in s2} {:trigger str in s1} :: str in s1 || str in s2 ==> str in s;
  assert forall str: Id {:trigger str in s} {:trigger str in s1} :: str in s1 ==> str in s;
  assert |s| >= |s1|;
  assert |s| >= |s1| && |s| >= |s2|;
}

function max(a: nat, b: nat): nat
  ensures var maxi: nat := max(a, b); maxi >= a && maxi >= b && (maxi == a || maxi == b)
  decreases a, b
{
  if a > b then
    a
...
function highestId(s: seq<Id>): nat
  ensures s != [] ==> forall index: nat {:trigger s[index]} :: index < |s| ==> s[index] <= highestId(s)
  ensures s != [] ==> exists index: nat {:trigger s[index]} :: index < |s| && s[index] == highestId(s)
...
function remove(s: seq<Id>, x: Id): seq<Id>
  ...
  ensures forall str: Id {:trigger str in s} {:trigger str in remove(s, x)} :: str in remove(s, x) ==> str in s
  ensures forall str: Id {:trigger str in remove(s, x)} {:trigger str in s} :: str in s && str != x ==> str in remove(s, x)
  ...
  decreases s, x
{
  if s == [] then
    ...
    assert allUnique(remove(s[1..], x)); assert !(s[0] in remove(s[1..], x)); elementNotInallUnique(remove(s[1..], x), s[0]); [s[0]] + remove(s[1..], x)
}
  ...
  decreases lHeight(t)
{
  ...
  case Lambda(y, t1) =>
    if y == x then
      ...
      Lambda(y, substitution(t1, x, t'))
  case Application(t1, t2) =>
    Application(substitution(t1, x, t'), substitution(t2, x, t'))
}

function caSubstitution(t: LambdaTerm, x: Id, t': LambdaTerm): LambdaTerm
  requires var s1: seq<Id> := vars(t); allUnique(s1)
  requires var s2: seq<Id> := vars(t'); allUnique(s2)
  decreases t, x, t'
{
  caSubstitution'(t, x, t', reunion(vars(t), vars(t')))
}

lemma /*{:_inductionTrigger substitution(t1, x, t2)}*/ /*{:_induction t1, x, t2}*/ subsitutionOfVarDoesNotChangeHeightFORVARIABLES(t1: LambdaTerm, x: Id, t2: LambdaTerm)
  requires match t2 case Var(x) => true case _ /* _v1 */ => false
  ensures lHeight(substitution(t1, x, t2)) == lHeight(t1)
  decreases t1, x, t2
{
  ghost var t' := substitution(t1, x, t2);
  match t1 {
    case {:split false} Var(y) =>
      {
        assert lHeight(t') == lHeight(t1);
      }
    case {:split false} Lambda(y, t) =>
      {
        if y == x {
          assert t' == t1;
        } else {
    ...
    case {:split false} Application(t1, t2) =>
      {
      }
  }
}
  ...
  decreases lHeight(t)
{
  ...
  case Lambda(y, t1) =>
    if y == x then
      t
    else if !(y in free(t')) then
      Lambda(y, caSubstitution'(t1, x, t', ids))
    else
      (var ids': seq<Id> := addAnUniqueId(ids); var vari: LambdaTerm := Var(ids'[0]); var t2: LambdaTerm := substitution(t1, y, vari); subsitutionOfVarDoesNotChangeHeightFORVARIABLES(t1, y, vari); Lambda(ids'[0], caSubstitution'(t2, x, t', ids')))
  case Application(t1, t2) =>
    Application(caSubstitution'(t1, x, t', ids), caSubstitution'(t2, x, t', ids))
}

lemma /*{:_inductionTrigger vars(t)}*/ /*{:_induction t}*/ ASubLambdaOfAUniqueLambdaIsUnique(t: LambdaTerm)
  requires allUnique(vars(t))
  ensures match t case Var(_ /* _v2 */) => true case Application(t1, t2) => allUnique(vars(t1)) && allUnique(vars(t2)) case Lambda(_ /* _v3 */, t') => allUnique(vars(t'))
  decreases t
{
}

function betaReduction(t: LambdaTerm): Option<LambdaTerm>
  requires var s1: seq<Id> := vars(t); allUnique(s1)
  decreases t
{
  match t
  case Var(x) =>
    None
  case Application(Lambda(x, t1), t') =>
    Some(caSubstitution(t1, x, t'))
  case Application(t1, t2) =>
    (match betaReduction(t1)
    case None() =>
      (match betaReduction(t2)
      case None() =>
        None
      case Some(t2') =>
        Some(Application(t1, t2')))
    case Some(t1') =>
      Some(Application(t1', t2)))
  case Lambda(x, t') =>
    match betaReduction(t')
    case None() =>
      None
    case Some(t2) =>
      Some(Lambda(x, t2))
...
method Main(_noArgsParameter: seq<seq<char>>)
{
  ...
  var result := betaReduction(testing);
  print result;
}
...
namespace _module {
  ...
  public partial class __default {
    public static Dafny.ISequence<BigInteger> bound(_ILambdaTerm t) {
      _ILambdaTerm _source0 = t;
      {
        if (_source0.is_Var) {
      ...
      }
      {
        if (_source0.is_Lambda) {
          BigInteger _0_x = _source0.dtor_x;
          _ILambdaTerm _1_t = _source0.dtor_t;
          return __default.addition(_0_x, __default.bound(_1_t));
        }
      }
      {
        _ILambdaTerm _2_t1 = _source0.dtor_t1;
        _ILambdaTerm _3_t2 = _source0.dtor_t2;
        return __default.reunion(__default.bound(_2_t1), __default.bound(_3_t2));
      }
    }
    public static Dafny.ISequence<BigInteger> addition(BigInteger x, Dafny.ISequence<BigInteger> s)
    {
      if ((s).Contains(x)) {
        return s;
      } else {
        return Dafny.Sequence<BigInteger>.Concat(s, Dafny.Sequence<BigInteger>.FromElements(x));
      }
    }
    public static Dafny.ISequence<BigInteger> reunion(Dafny.ISequence<BigInteger> s1, Dafny.ISequence<BigInteger> s2)
    {
      Dafny.ISequence<BigInteger> _0___accumulator = Dafny.Sequence<BigInteger>.FromElements();
    TAIL_CALL_START: ;
      if ((s1).Equals(Dafny.Sequence<BigInteger>.FromElements())) {
        return Dafny.Sequence<BigInteger>.Concat(_0___accumulator, s2);
      } else {
        _0___accumulator = Dafny.Sequence<BigInteger>.Concat(_0___accumulator, Dafny.Sequence<BigInteger>.FromElements((s1).Select(BigInteger.Zero)));
        Dafny.ISequence<BigInteger> _in0 = __default.@remove(s1, (s1).Select(BigInteger.Zero));
        Dafny.ISequence<BigInteger> _in1 = __default.@remove(s2, (s1).Select(BigInteger.Zero));
        s1 = _in0;
        s2 = _in1;
    ...
    public static BigInteger highestId(Dafny.ISequence<BigInteger> s) {
      if ((s).Equals(Dafny.Sequence<BigInteger>.FromElements())) {
        return BigInteger.Zero;
      } else {
        return __default.max((s).Select(BigInteger.Zero), __default.highestId((s).Drop(BigInteger.One)));
      }
    }
    public static Dafny.ISequence<BigInteger> addAnUniqueId(Dafny.ISequence<BigInteger> s) {
      if ((s).Equals(Dafny.Sequence<BigInteger>.FromElements())) {
        return Dafny.Sequence<BigInteger>.FromElements(BigInteger.Zero);
      } else {
        return Dafny.Sequence<BigInteger>.Concat(Dafny.Sequence<BigInteger>.FromElements((__default.highestId(s)) + (BigInteger.One)), s);
      }
    }
    public static Dafny.ISequence<BigInteger> @remove(Dafny.ISequence<BigInteger> s, BigInteger x)
    {
      Dafny.ISequence<BigInteger> _0___accumulator = Dafny.Sequence<BigInteger>.FromElements();
    TAIL_CALL_START: ;
      if ((s).Equals(Dafny.Sequence<BigInteger>.FromElements())) {
        return Dafny.Sequence<BigInteger>.Concat(_0___accumulator, Dafny.Sequence<BigInteger>.FromElements());
      } else if (((s).Select(BigInteger.Zero)) == (x)) {
        return Dafny.Sequence<BigInteger>.Concat(_0___accumulator, (s).Drop(BigInteger.One));
      } else {
    ...
    public static Dafny.ISequence<BigInteger> free(_ILambdaTerm t) {
      _ILambdaTerm _source0 = t;
      {
        if (_source0.is_Var) {
          BigInteger _0_x = _source0.dtor_id;
          return Dafny.Sequence<BigInteger>.FromElements(_0_x);
        }
      }
      {
        if (_source0.is_Lambda) {
          BigInteger _1_x = _source0.dtor_x;
          _ILambdaTerm _2_t = _source0.dtor_t;
          return __default.@remove(__default.free(_2_t), _1_x);
        }
      }
      {
        _ILambdaTerm _3_t1 = _source0.dtor_t1;
        _ILambdaTerm _4_t2 = _source0.dtor_t2;
        return __default.reunion(__default.free(_3_t1), __default.free(_4_t2));
      }
    }
    public static Dafny.ISequence<BigInteger> vars(_ILambdaTerm t) {
      _ILambdaTerm _source0 = t;
      {
        if (_source0.is_Var) {
          BigInteger _0_x = _source0.dtor_id;
      ...
      }
      {
        if (_source0.is_Lambda) {
          BigInteger _1_x = _source0.dtor_x;
          _ILambdaTerm _2_t_k = _source0.dtor_t;
          return __default.addition(_1_x, __default.vars(_2_t_k));
        }
      }
      {
        _ILambdaTerm _3_t1 = _source0.dtor_t1;
        _ILambdaTerm _4_t2 = _source0.dtor_t2;
        return __default.reunion(__default.vars(_3_t1), __default.vars(_4_t2));
      }
    }
    public static BigInteger lHeight(_ILambdaTerm t) {
      _ILambdaTerm _source0 = t;
      {
        if (_source0.is_Var) {
          BigInteger _0_x = _source0.dtor_id;
      ...
      }
      {
        if (_source0.is_Lambda) {
          BigInteger _1_x = _source0.dtor_x;
          _ILambdaTerm _2_t_k = _source0.dtor_t;
          return (BigInteger.One) + (__default.lHeight(_2_t_k));
        }
      }
      {
        _ILambdaTerm _3_t1 = _source0.dtor_t1;
        _ILambdaTerm _4_t2 = _source0.dtor_t2;
        return (BigInteger.One) + (__default.max(__default.lHeight(_3_t1), __default.lHeight(_4_t2)));
      }
    }
    public static _ILambdaTerm substitution(_ILambdaTerm t, BigInteger x, _ILambdaTerm t_k)
    {
      _ILambdaTerm _source0 = t;
      {
        if (_source0.is_Var) {
          BigInteger _0_y = _source0.dtor_id;
          if ((_0_y) == (x)) {
            return t_k;
          } else {
            return _module.LambdaTerm.create_Var(_0_y);
          }
        }
      }
      {
        if (_source0.is_Lambda) {
          BigInteger _1_y = _source0.dtor_x;
          _ILambdaTerm _2_t1 = _source0.dtor_t;
          if ((_1_y) == (x)) {
            return t;
          } else {
            return _module.LambdaTerm.create_Lambda(_1_y, __default.substitution(_2_t1, x, t_k));
          }
        }
      }
      {
        _ILambdaTerm _3_t1 = _source0.dtor_t1;
        _ILambdaTerm _4_t2 = _source0.dtor_t2;
        return _module.LambdaTerm.create_Application(__default.substitution(_3_t1, x, t_k), __default.substitution(_4_t2, x, t_k));
      }
    }
    public static _ILambdaTerm caSubstitution(_ILambdaTerm t, BigInteger x, _ILambdaTerm t_k)
    {
      return __default.caSubstitution_k(t, x, t_k, __default.reunion(__default.vars(t), __default.vars(t_k)));
    }
    public static _ILambdaTerm caSubstitution_k(_ILambdaTerm t, BigInteger x, _ILambdaTerm t_k, Dafny.ISequence<BigInteger> ids)
    {
      _ILambdaTerm _source0 = t;
      {
        if (_source0.is_Var) {
      ...
      }
      {
        if (_source0.is_Lambda) {
          ...
          if ((_1_y) == (x)) {
            return t;
          } else if (!((__default.free(t_k)).Contains(_1_y))) {
            return _module.LambdaTerm.create_Lambda(_1_y, __default.caSubstitution_k(_2_t1, x, t_k, ids));
          } else {
            Dafny.ISequence<BigInteger> _3_ids_k = __default.addAnUniqueId(ids);
            _ILambdaTerm _4_vari = _module.LambdaTerm.create_Var((_3_ids_k).Select(BigInteger.Zero));
            _ILambdaTerm _5_t2 = __default.substitution(_2_t1, _1_y, _4_vari);
            return _module.LambdaTerm.create_Lambda((_3_ids_k).Select(BigInteger.Zero), __default.caSubstitution_k(_5_t2, x, t_k, _3_ids_k));
          }
        }
      }
      {
        _ILambdaTerm _6_t1 = _source0.dtor_t1;
        _ILambdaTerm _7_t2 = _source0.dtor_t2;
        return _module.LambdaTerm.create_Application(__default.caSubstitution_k(_6_t1, x, t_k, ids), __default.caSubstitution_k(_7_t2, x, t_k, ids));
      }
    }
    public static _IOption<_ILambdaTerm> betaReduction(_ILambdaTerm t) {
      _ILambdaTerm _source0 = t;
      {
        if (_source0.is_Var) {
          BigInteger _0_x = _source0.dtor_id;
          return _module.Option<_ILambdaTerm>.create_None();
        }
      }
      {
        if (_source0.is_Application) {
          _ILambdaTerm t10 = _source0.dtor_t1;
          if (t10.is_Lambda) {
            BigInteger _1_x = t10.dtor_x;
            _ILambdaTerm _2_t1 = t10.dtor_t;
            _ILambdaTerm _3_t_k = _source0.dtor_t2;
            return _module.Option<_ILambdaTerm>.create_Some(__default.caSubstitution(_2_t1, _1_x, _3_t_k));
          }
        }
      }
      {
        if (_source0.is_Application) {
          _ILambdaTerm _4_t1 = _source0.dtor_t1;
          _ILambdaTerm _5_t2 = _source0.dtor_t2;
          _IOption<_ILambdaTerm> _source1 = __default.betaReduction(_4_t1);
          {
            if (_source1.is_None) {
              _IOption<_ILambdaTerm> _source2 = __default.betaReduction(_5_t2);
              {
                if (_source2.is_None) {
                  return _module.Option<_ILambdaTerm>.create_None();
                }
              ...
              {
                _ILambdaTerm _6_t2_k = _source2.dtor_t;
                return _module.Option<_ILambdaTerm>.create_Some(_module.LambdaTerm.create_Application(_4_t1, _6_t2_k));
              }
            }
          }
          {
            _ILambdaTerm _7_t1_k = _source1.dtor_t;
            return _module.Option<_ILambdaTerm>.create_Some(_module.LambdaTerm.create_Application(_7_t1_k, _5_t2));
          }
        }
      }
      {
        BigInteger _8_x = _source0.dtor_x;
        _ILambdaTerm _9_t_k = _source0.dtor_t;
        _IOption<_ILambdaTerm> _source3 = __default.betaReduction(_9_t_k);
        {
          if (_source3.is_None) {
            return _module.Option<_ILambdaTerm>.create_None();
          }
        ...
        {
          _ILambdaTerm _10_t2 = _source3.dtor_t;
          return _module.Option<_ILambdaTerm>.create_Some(_module.LambdaTerm.create_Lambda(_8_x, _10_t2));
        }
      }
    }
    public static void _Main(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> __noArgsParameter)
    {
      BigInteger _0_x;
      _0_x = BigInteger.One;
      _ILambdaTerm _1_testing;
      _1_testing = _module.LambdaTerm.create_Application(_module.LambdaTerm.create_Lambda(_0_x, _module.LambdaTerm.create_Application(_module.LambdaTerm.create_Var(_0_x), _module.LambdaTerm.create_Var(_0_x))), _module.LambdaTerm.create_Lambda(_0_x, _module.LambdaTerm.create_Application(_module.LambdaTerm.create_Var(_0_x), _module.LambdaTerm.create_Var(_0_x))));
      _IOption<_ILambdaTerm> _2_result;
      _2_result = __default.betaReduction(_1_testing);
      Dafny.Helpers.Print((_2_result));
    }
    ...
    public static Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> ReservedWords { get {
      return Dafny.Sequence<Dafny.ISequence<Dafny.Rune>>.FromElements(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("AND"), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("OR"), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("NOT"), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("TRUE"), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("FALSE"), Dafny.Sequence<Dafny.Rune>.UnicodeFromString("CERTIFICATE"));
    } }
  }

  public interface _ILambdaTerm {
    bool is_Var { get; }
    bool is_Lambda { get; }
    bool is_Application { get; }
    BigInteger dtor_id { get; }
    BigInteger dtor_x { get; }
    _ILambdaTerm dtor_t { get; }
    _ILambdaTerm dtor_t1 { get; }
    _ILambdaTerm dtor_t2 { get; }
    _ILambdaTerm DowncastClone();
  }
  public abstract class LambdaTerm : _ILambdaTerm {
    public LambdaTerm() {
    }
    private static readonly _ILambdaTerm theDefault = create_Var(BigInteger.Zero);
    public static _ILambdaTerm Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_ILambdaTerm> _TYPE = new Dafny.TypeDescriptor<_ILambdaTerm>(LambdaTerm.Default());
    public static Dafny.TypeDescriptor<_ILambdaTerm> _TypeDescriptor() {
      return _TYPE;
    }
    public static _ILambdaTerm create_Var(BigInteger id) {
      return new LambdaTerm_Var(id);
    }
    public static _ILambdaTerm create_Lambda(BigInteger x, _ILambdaTerm t) {
      return new LambdaTerm_Lambda(x, t);
    }
    public static _ILambdaTerm create_Application(_ILambdaTerm t1, _ILambdaTerm t2) {
      return new LambdaTerm_Application(t1, t2);
    }
    public bool is_Var { get { return this is LambdaTerm_Var; } }
    public bool is_Lambda { get { return this is LambdaTerm_Lambda; } }
    public bool is_Application { get { return this is LambdaTerm_Application; } }
    public BigInteger dtor_id {
      get {
        var d = this;
        return ((LambdaTerm_Var)d)._id;
      }
    }
    public BigInteger dtor_x {
      get {
        var d = this;
        return ((LambdaTerm_Lambda)d)._x;
      }
    }
    public _ILambdaTerm dtor_t {
      get {
        var d = this;
        return ((LambdaTerm_Lambda)d)._t;
      }
    }
    public _ILambdaTerm dtor_t1 {
      get {
        var d = this;
        return ((LambdaTerm_Application)d)._t1;
      }
    }
    public _ILambdaTerm dtor_t2 {
      get {
        var d = this;
        return ((LambdaTerm_Application)d)._t2;
      }
    }
    public abstract _ILambdaTerm DowncastClone();
  }
  public class LambdaTerm_Var : LambdaTerm {
    public readonly BigInteger _id;
    public LambdaTerm_Var(BigInteger id) : base() {
      this._id = id;
    }
    public override _ILambdaTerm DowncastClone() {
      if (this is _ILambdaTerm dt) { return dt; }
      return new LambdaTerm_Var(_id);
    }
    public override bool Equals(object other) {
      var oth = other as LambdaTerm_Var;
  ...
  public class LambdaTerm_Lambda : LambdaTerm {
    public readonly BigInteger _x;
    public readonly _ILambdaTerm _t;
    public LambdaTerm_Lambda(BigInteger x, _ILambdaTerm t) : base() {
      this._x = x;
      this._t = t;
    }
    public override _ILambdaTerm DowncastClone() {
      if (this is _ILambdaTerm dt) { return dt; }
      return new LambdaTerm_Lambda(_x, _t);
    }
    public override bool Equals(object other) {
      var oth = other as LambdaTerm_Lambda;
      return oth != null && this._x == oth._x && object.Equals(this._t, oth._t);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 1;
  ...
  public class LambdaTerm_Application : LambdaTerm {
    public readonly _ILambdaTerm _t1;
    public readonly _ILambdaTerm _t2;
    public LambdaTerm_Application(_ILambdaTerm t1, _ILambdaTerm t2) : base() {
      this._t1 = t1;
      this._t2 = t2;
    }
    public override _ILambdaTerm DowncastClone() {
      if (this is _ILambdaTerm dt) { return dt; }
      return new LambdaTerm_Application(_t1, _t2);
    }
    public override bool Equals(object other) {
      var oth = other as LambdaTerm_Application;
      return oth != null && object.Equals(this._t1, oth._t1) && object.Equals(this._t2, oth._t2);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 2;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._t1));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._t2));
      return (int) hash;
    }
    public override string ToString() {
      string s = "LambdaTerm.Application";
      s += "(";
      s += Dafny.Helpers.ToString(this._t1);
      s += ", ";
      s += Dafny.Helpers.ToString(this._t2);
      s += ")";
      return s;
    }
  }
  ...
  public interface _IcircuitCell {
    ...
    BigInteger dtor_pos1 { get; }
    BigInteger dtor_pos2 { get; }
  ...
  public abstract class circuitCell : _IcircuitCell {
    ...
    private static readonly _IcircuitCell theDefault = create_AND(BigInteger.Zero, BigInteger.Zero);
    ...
    public static _IcircuitCell create_AND(BigInteger pos1, BigInteger pos2) {
      return new circuitCell_AND(pos1, pos2);
    }
    public static _IcircuitCell create_OR(BigInteger pos1, BigInteger pos2) {
      return new circuitCell_OR(pos1, pos2);
    }
    public static _IcircuitCell create_NOT(BigInteger pos) {
      return new circuitCell_NOT(pos);
    }
    ...
    public BigInteger dtor_pos1 {
      get {
        var d = this;
        if (d is circuitCell_AND) { return ((circuitCell_AND)d)._pos1; }
        return ((circuitCell_OR)d)._pos1;
      }
    }
    public BigInteger dtor_pos2 {
      get {
        var d = this;
        if (d is circuitCell_AND) { return ((circuitCell_AND)d)._pos2; }
        return ((circuitCell_OR)d)._pos2;
      }
    }
    public BigInteger dtor_pos {
      get {
        var d = this;
        return ((circuitCell_NOT)d)._pos;
      }
    }
  ...
  public class circuitCell_AND : circuitCell {
    public readonly BigInteger _pos1;
    public readonly BigInteger _pos2;
    public circuitCell_AND(BigInteger pos1, BigInteger pos2) : base() {
      this._pos1 = pos1;
      this._pos2 = pos2;
    }
    public override _IcircuitCell DowncastClone() {
      if (this is _IcircuitCell dt) { return dt; }
      return new circuitCell_AND(_pos1, _pos2);
    }
    public override bool Equals(object other) {
      var oth = other as circuitCell_AND;
      return oth != null && this._pos1 == oth._pos1 && this._pos2 == oth._pos2;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._pos1));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._pos2));
      return (int) hash;
    }
    public override string ToString() {
      string s = "circuitCell.AND";
      s += "(";
      s += Dafny.Helpers.ToString(this._pos1);
      s += ", ";
      s += Dafny.Helpers.ToString(this._pos2);
      s += ")";
      return s;
    }
  }
  public class circuitCell_OR : circuitCell {
    public readonly BigInteger _pos1;
    public readonly BigInteger _pos2;
    public circuitCell_OR(BigInteger pos1, BigInteger pos2) : base() {
      this._pos1 = pos1;
      this._pos2 = pos2;
    }
    public override _IcircuitCell DowncastClone() {
      if (this is _IcircuitCell dt) { return dt; }
      return new circuitCell_OR(_pos1, _pos2);
    }
    public override bool Equals(object other) {
  ...
  public class circuitCell_NOT : circuitCell {
    ...
    public circuitCell_NOT(BigInteger pos) : base() {
      this._pos = pos;
    }
    public override _IcircuitCell DowncastClone() {
      if (this is _IcircuitCell dt) { return dt; }
      return new circuitCell_NOT(_pos);
    }
    public override bool Equals(object other) {
