// Dafny program CircuitSATEncoding.dfy compiled into C#
...
// Command-line arguments: run --no-verify c:\Users\Under\Licenta\Circuit-SAT\CircuitSATEncoding.dfy
// CircuitSATEncoding.dfy
...
lemma /*{:_inductionTrigger natToString(n)}*/ /*{:_induction n}*/ natToStringThenStringToNatIdem(n: nat)
  ensures stringToNat(natToString(n)) == n
  decreases n
{
}

lemma /*{:_inductionTrigger stringToNat(n)}*/ /*{:_inductionTrigger isStringAValidNumber(n)}*/ /*{:_induction n}*/ stringToNatThenNatToStringIdem(n: string)
...
predicate isCktValid(c: circuit)
  decreases c
{
  forall pos: nat, _t#0: int, _t#1: int {:trigger c[_t#1], c[_t#0], c[pos]} | _t#1 == pos + 2 && _t#0 == pos + 1 :: 
    0 <= pos &&
    pos < |c| ==>
      match c[pos] case AND() => pos < |c| - 2 && isCircuitCellAValidPoz(c[_t#0], pos, c) && isCircuitCellAValidPoz(c[_t#1], pos, c) case OR() => pos < |c| - 2 && isCircuitCellAValidPoz(c[_t#0], pos, c) && isCircuitCellAValidPoz(c[_t#1], pos, c) case NOT() => pos < |c| - 1 && isCircuitCellAValidPoz(c[_t#0], pos, c) case VARIABLE(_ /* _v3 */) => true case POZ(nr) => pos > 0 && 0 < nr && nr < pos - 1 && pos - nr - 1 >= 0
}

predicate isCircuitValid(c: circuit)
  decreases c
{
  forall pos: nat {:trigger isCircuitValid'(c, pos)} :: 
    0 <= pos < |c| ==>
      isCircuitValid'(c, pos)
}
  ...
  decreases c, pos
{
  ...
  case AND() =>
    pos < |c| - 2 &&
    isCircuitCellAValidPoz(c[pos + 1], pos, c) &&
    isCircuitCellAValidPoz(c[pos + 2], pos, c)
  case OR() =>
    pos < |c| - 2 &&
    isCircuitCellAValidPoz(c[pos + 1], pos, c) &&
    isCircuitCellAValidPoz(c[pos + 2], pos, c)
  ...
  decreases |c| - poz
{
  if poz == |c| then
    ...
    match c[poz] case AND() => [""AND""] + circuitToInput'(c, poz + 1) case OR() => [""OR""] + circuitToInput'(c, poz + 1) case NOT() => [""NOT""] + circuitToInput'(c, poz + 1) case VARIABLE(x) => [x] + circuitToInput'(c, poz + 1) case POZ(nr) => assert isCircuitValid'(c, poz); assert 0 < nr < poz - 1; assert poz - nr - 1 > 0; [natToString(poz - nr - 1)] + circuitToInput'(c, poz + 1)
}
  ...
  decreases |input| - pos
{
  if pos == |input| then
    ...
    match stringToCircuitCell(input[pos], pos) case Some(g) => (match inputToCircuit'(input, pos + 1) case Some(l) => Some([g] + l) case None() => None) case None() => None
}
  ...
  decreases s, pos
{
  if s == ""AND"" then
    ...
    Some(NOT)
  else if isStringAValidNumber(s) then
  ...
  decreases k
{
  |k| > 0 &&
  |k| % 2 == 0 &&
  forall pos: nat {:trigger k[pos]} :: 
    pos < |k| ==>
      if pos % 2 == 0 then k[pos] != ""true"" && k[pos] != ""false"" else k[pos] == ""true"" || k[pos] == ""false""
}
...
predicate isCertificateValid(c: circuit, k: certificate)
  ...
  decreases c, k
{
  forall pos: nat {:trigger variableHasValue(c, k, pos)} :: 
    pos < |c| ==>
      variableHasValue(c, k, pos)
}
  ...
  decreases c, k, pos
{
  match c[pos]
  case VARIABLE(x) =>
    exists poz: nat, _t#0: int {:trigger k[_t#0], k[poz]} | _t#0 == poz + 1 :: 
      poz < |k| - 1 &&
      k[poz] == x &&
      k[_t#0] == ""true"" &&
      k[_t#0] == ""false""
...
function getValueFromCertificate'(v: string, k: certificate, pos: nat): Option<bool>
  requires pos < |k|
  requires v != ""false"" && v != ""true""
  requires forall poz: nat {:trigger k[poz]} :: poz < pos ==> k[poz] != v
  requires isCertificateCorrectForm(k)
  ensures getValueFromCertificate'(v, k, pos) != None <==> exists poz: nat, _t#0: int {:trigger k[_t#0], k[poz]} | _t#0 == poz + 1 :: poz < |k| - 1 && k[poz] == v && (k[_t#0] == ""true"" || k[_t#0] == ""false"")
  ensures forall poz: nat {:trigger k[poz]} :: poz < pos ==> k[poz] != v
  decreases |k| - pos
{
  if pos == |k| - 1 then
    None
  else if k[pos] == v then
  ...
  decreases pos
{
  match c[pos]
  case AND() =>
    assert isCircuitValid'(c, pos); solveCircuit'(c, k, getNumberFromPoz(c[pos + 1])) && solveCircuit'(c, k, getNumberFromPoz(c[pos + 2]))
  case OR() =>
    assert isCircuitValid'(c, pos); solveCircuit'(c, k, getNumberFromPoz(c[pos + 1])) || solveCircuit'(c, k, getNumberFromPoz(c[pos + 2]))
  case NOT() =>
    assert isCircuitValid'(c, pos); !solveCircuit'(c, k, getNumberFromPoz(c[pos + 1]))
  case VARIABLE(x) =>
    assert isCircuitValid'(c, pos); match getValueFromCertificate(x, k) case Some(st) => st case None() => false
  case POZ(nr) =>
...
ghost predicate checkSatisfaction(c: circuit)
  ...
  decreases c
{
  exists k: certificate {:trigger solveCircuit(c, k)} {:trigger isCertificateValid(c, k)} {:trigger isCertificateCorrectForm(k)} :: 
    isCertificateCorrectForm(k) &&
    isCertificateValid(c, k) &&
    |c| >= 1 &&
    solveCircuit(c, k)
}
...
ghost predicate isThatActuallyTheCKTSATLanguage(l: Language)
  decreases l
{
  (forall input: seq<string> {:trigger inputToCircuit(input)} {:trigger input in l} :: 
    input in l ==>
      match inputToCircuit(input) case Some(c) => isCircuitValid(c) && checkSatisfaction(c) case None() => false) &&
  forall c: circuit {:trigger circuitToInput(c)} {:trigger checkSatisfaction(c)} {:trigger isCircuitValid(c)} :: 
    isCircuitValid(c) &&
    checkSatisfaction(c) ==>
  ...
  case Configuration(s, tape, head) =>
    if Key(s, tape(head)) in delta.Keys then
      ...
      case Action(state, symbol, direction) =>
        match direction
        case Left() =>
          Some(moveLeft(Configuration(state, changeSymbol(tape, head, symbol), head)))
        case Right() =>
          Some(moveRight(Configuration(state, changeSymbol(tape, head, symbol), head)))
    else
      None
}

function changeSymbol(tape: Tape, head: int, newSymbol: Symbol): Tape
  decreases head, newSymbol
{
  (i: int) => if i != head then tape(i) else newSymbol
}
...
type stringNat = s: string
  | |s| > 0 && (|s| > 1 ==> s[0] != '0') && forall i: int | 0 <= i < |s| :: s[i] in ""0123456789""
  witness ""1""
...
namespace _module {
  ...
  public partial class __default {
    public static bool isStringAValidNumber(Dafny.ISequence<Dafny.Rune> s) {
      return (((new BigInteger((s).Count)).Sign == 1) && (!((new BigInteger((s).Count)) > (BigInteger.One)) || (((s).Select(BigInteger.Zero)) != (new Dafny.Rune('0'))))) && (Dafny.Helpers.Id<Func<Dafny.ISequence<Dafny.Rune>, bool>>((_0_s) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_0_s).Count)), true, (((_forall_var_0) => {
        BigInteger _1_i = (BigInteger)_forall_var_0;
        return !(((_1_i).Sign != -1) && ((_1_i) < (new BigInteger((_0_s).Count)))) || ((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("0123456789")).Contains((_0_s).Select(_1_i)));
      }))))(s));
    }
    public static Dafny.ISequence<Dafny.Rune> natToString(BigInteger n) {
      ...
      {
        return Dafny.Sequence<Dafny.Rune>.Concat(__default.natToString(Dafny.Helpers.EuclideanDivision(n, new BigInteger(10))), __default.natToString(Dafny.Helpers.EuclideanModulus(n, new BigInteger(10))));
      }
    }
    public static BigInteger stringToNat(Dafny.ISequence<Dafny.Rune> s) {
      if ((new BigInteger((s).Count)) == (BigInteger.One)) {
        Dafny.Rune _source0 = (s).Select(BigInteger.Zero);
        {
        ...
        return ((__default.stringToNat((s).Take((new BigInteger((s).Count)) - (BigInteger.One)))) * (new BigInteger(10))) + (__default.stringToNat((s).Subsequence((new BigInteger((s).Count)) - (BigInteger.One), new BigInteger((s).Count))));
      }
    }
    public static bool isCircuitCellAPoz(_IcircuitCell e) {
      _IcircuitCell _source0 = e;
      {
        if (_source0.is_POZ) {
          BigInteger _0_nr = _source0.dtor_i;
          return true;
        }
      }
      {
    ...
    public static bool isCircuitCellAValidPoz(_IcircuitCell e, BigInteger poz, Dafny.ISequence<_IcircuitCell> c)
    {
      _IcircuitCell _source0 = e;
      {
        if (_source0.is_POZ) {
          BigInteger _0_nr = _source0.dtor_i;
          if (((_0_nr).Sign != -1) && ((_0_nr) < ((poz) - (BigInteger.One)))) {
            return !(__default.isCircuitCellAPoz((c).Select(_0_nr)));
          } else {
            return false;
          }
        }
    ...
    public static bool isCktValid(Dafny.ISequence<_IcircuitCell> c) {
      return Dafny.Helpers.Id<Func<Dafny.ISequence<_IcircuitCell>, bool>>((_0_c) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_0_c).Count)), true, (((_forall_var_0) => {
        BigInteger _1_pos = (BigInteger)_forall_var_0;
        if (_System.nat._Is(_1_pos)) {
          return !(((_1_pos).Sign != -1) && ((_1_pos) < (new BigInteger((_0_c).Count)))) || (((System.Func<bool>)(() => {
            _IcircuitCell _source0 = (_0_c).Select(_1_pos);
            {
              if (_source0.is_AND) {
                return (((_1_pos) < ((new BigInteger((_0_c).Count)) - (new BigInteger(2)))) && (__default.isCircuitCellAValidPoz((_0_c).Select((_1_pos) + (BigInteger.One)), _1_pos, _0_c))) && (__default.isCircuitCellAValidPoz((_0_c).Select((_1_pos) + (new BigInteger(2))), _1_pos, _0_c));
              }
            }
            {
              if (_source0.is_OR) {
            ...
            {
              if (_source0.is_NOT) {
                return ((_1_pos) < ((new BigInteger((_0_c).Count)) - (BigInteger.One))) && (__default.isCircuitCellAValidPoz((_0_c).Select((_1_pos) + (BigInteger.One)), _1_pos, _0_c));
              }
            }
            {
              if (_source0.is_VARIABLE) {
                return true;
              }
            }
            {
              BigInteger _2_nr = _source0.dtor_i;
              return (((_1_pos).Sign == 1) && (((_2_nr).Sign == 1) && ((_2_nr) < ((_1_pos) - (BigInteger.One))))) && ((((_1_pos) - (_2_nr)) - (BigInteger.One)).Sign != -1);
            }
          }))());
        } else {
          return true;
    ...
    public static bool isCircuitValid(Dafny.ISequence<_IcircuitCell> c) {
      return Dafny.Helpers.Id<Func<Dafny.ISequence<_IcircuitCell>, bool>>((_0_c) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_0_c).Count)), true, (((_forall_var_0) => {
        BigInteger _1_pos = (BigInteger)_forall_var_0;
        if (_System.nat._Is(_1_pos)) {
          return !(((_1_pos).Sign != -1) && ((_1_pos) < (new BigInteger((_0_c).Count)))) || (__default.isCircuitValid_k(_0_c, _1_pos));
        } else {
          return true;
        }
      }))))(c);
    }
    public static bool isCircuitValid_k(Dafny.ISequence<_IcircuitCell> c, BigInteger pos)
    {
      _IcircuitCell _source0 = (c).Select(pos);
      {
        if (_source0.is_AND) {
          return (((pos) < ((new BigInteger((c).Count)) - (new BigInteger(2)))) && (__default.isCircuitCellAValidPoz((c).Select((pos) + (BigInteger.One)), pos, c))) && (__default.isCircuitCellAValidPoz((c).Select((pos) + (new BigInteger(2))), pos, c));
        }
      }
      {
        if (_source0.is_OR) {
      ...
      {
        if (_source0.is_NOT) {
          return ((pos) < ((new BigInteger((c).Count)) - (BigInteger.One))) && (__default.isCircuitCellAValidPoz((c).Select((pos) + (BigInteger.One)), pos, c));
        }
      }
      {
        if (_source0.is_VARIABLE) {
          Dafny.ISequence<Dafny.Rune> _0_x = _source0.dtor_x;
          return (!(_0_x).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("true"))) && (!(_0_x).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("false")));
        }
      }
      {
        BigInteger _1_nr = _source0.dtor_i;
        return ((((pos).Sign == 1) && (((_1_nr).Sign == 1) && ((_1_nr) < ((pos) - (BigInteger.One))))) && ((((pos) - (_1_nr)) - (BigInteger.One)).Sign != -1)) && (!(__default.isCircuitCellAPoz((c).Select(_1_nr))));
      }
    }
    public static _IOption<Dafny.ISequence<_IcircuitCell>> inputToCircuit(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> input) {
      return __default.inputToCircuit_k(input, BigInteger.Zero);
    }
    public static _IOption<Dafny.ISequence<_IcircuitCell>> inputToCircuit_k(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> input, BigInteger pos)
    {
      if ((pos) == (new BigInteger((input).Count))) {
        return _module.Option<Dafny.ISequence<_IcircuitCell>>.create_Some(Dafny.Sequence<_IcircuitCell>.FromElements());
      } else {
        _IOption<_IcircuitCell> _source0 = __default.stringToCircuitCell((input).Select(pos), pos);
        {
          if (_source0.is_Some) {
            _IcircuitCell _0_g = _source0.dtor_t;
            _IOption<Dafny.ISequence<_IcircuitCell>> _source1 = __default.inputToCircuit_k(input, (pos) + (BigInteger.One));
            {
              if (_source1.is_Some) {
                Dafny.ISequence<_IcircuitCell> _1_l = _source1.dtor_t;
                return _module.Option<Dafny.ISequence<_IcircuitCell>>.create_Some(Dafny.Sequence<_IcircuitCell>.Concat(Dafny.Sequence<_IcircuitCell>.FromElements(_0_g), _1_l));
              }
            }
            {
              return _module.Option<Dafny.ISequence<_IcircuitCell>>.create_None();
            }
          }
        }
        {
    ...
    public static _IOption<_IcircuitCell> stringToCircuitCell(Dafny.ISequence<Dafny.Rune> s, BigInteger pos)
    {
      if ((s).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("AND"))) {
        return _module.Option<_IcircuitCell>.create_Some(_module.circuitCell.create_AND());
      } else if ((s).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("OR"))) {
        return _module.Option<_IcircuitCell>.create_Some(_module.circuitCell.create_OR());
      } else if ((s).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("NOT"))) {
        return _module.Option<_IcircuitCell>.create_Some(_module.circuitCell.create_NOT());
      } else if (__default.isStringAValidNumber(s)) {
        if ((((pos) - (__default.stringToNat(s))) - (BigInteger.One)).Sign != 1) {
          return _module.Option<_IcircuitCell>.create_None();
        } else {
          return _module.Option<_IcircuitCell>.create_Some(_module.circuitCell.create_POZ(((pos) - (__default.stringToNat(s))) - (BigInteger.One)));
        }
      } else {
        return _module.Option<_IcircuitCell>.create_Some(_module.circuitCell.create_VARIABLE(s));
      }
    }
    public static bool isCertificateCorrectForm(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> k) {
      return (((new BigInteger((k).Count)).Sign == 1) && ((Dafny.Helpers.EuclideanModulus(new BigInteger((k).Count), new BigInteger(2))).Sign == 0)) && (Dafny.Helpers.Id<Func<Dafny.ISequence<Dafny.ISequence<Dafny.Rune>>, bool>>((_0_k) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_0_k).Count)), true, (((_forall_var_0) => {
        BigInteger _1_pos = (BigInteger)_forall_var_0;
        if (_System.nat._Is(_1_pos)) {
          return !((_1_pos) < (new BigInteger((_0_k).Count))) || ((((Dafny.Helpers.EuclideanModulus(_1_pos, new BigInteger(2))).Sign == 0) ? ((!((_0_k).Select(_1_pos)).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("true"))) && (!((_0_k).Select(_1_pos)).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("false")))) : ((((_0_k).Select(_1_pos)).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("true"))) || (((_0_k).Select(_1_pos)).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("false"))))));
        } else {
          return true;
        }
      }))))(k));
    }
    public static bool isCertificateValid(Dafny.ISequence<_IcircuitCell> c, Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> k)
    {
      return Dafny.Helpers.Id<Func<Dafny.ISequence<_IcircuitCell>, Dafny.ISequence<Dafny.ISequence<Dafny.Rune>>, bool>>((_0_c, _1_k) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_0_c).Count)), true, (((_forall_var_0) => {
        BigInteger _2_pos = (BigInteger)_forall_var_0;
        if (_System.nat._Is(_2_pos)) {
          return !((_2_pos) < (new BigInteger((_0_c).Count))) || (__default.variableHasValue(_0_c, _1_k, _2_pos));
        } else {
          return true;
        }
      }))))(c, k);
    }
    public static bool variableHasValue(Dafny.ISequence<_IcircuitCell> c, Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> k, BigInteger pos)
    {
      _IcircuitCell _source0 = (c).Select(pos);
      {
        if (_source0.is_VARIABLE) {
          Dafny.ISequence<Dafny.Rune> _0_x = _source0.dtor_x;
          return Dafny.Helpers.Id<Func<Dafny.ISequence<Dafny.ISequence<Dafny.Rune>>, Dafny.ISequence<Dafny.Rune>, bool>>((_1_k, _2_x) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, (new BigInteger((_1_k).Count)) - (BigInteger.One)), false, (((_exists_var_0) => {
            BigInteger _3_poz = (BigInteger)_exists_var_0;
            if (_System.nat._Is(_3_poz)) {
              return ((_3_poz) < ((new BigInteger((_1_k).Count)) - (BigInteger.One))) && ((((_1_k).Select(_3_poz)).Equals(_2_x)) && ((((_1_k).Select((_3_poz) + (BigInteger.One))).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("true"))) && (((_1_k).Select((_3_poz) + (BigInteger.One))).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("false")))));
            } else {
              return false;
            }
          }))))(k, _0_x);
    ...
    public static _IOption<bool> getValueFromCertificate(Dafny.ISequence<Dafny.Rune> v, Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> k)
    {
      return __default.getValueFromCertificate_k(v, k, BigInteger.Zero);
    }
    public static _IOption<bool> getValueFromCertificate_k(Dafny.ISequence<Dafny.Rune> v, Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> k, BigInteger pos)
    {
    TAIL_CALL_START: ;
      if ((pos) == ((new BigInteger((k).Count)) - (BigInteger.One))) {
        return _module.Option<bool>.create_None();
      } else if (((k).Select(pos)).Equals(v)) {
        if (((k).Select((pos) + (BigInteger.One))).Equals(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("true"))) {
          return _module.Option<bool>.create_Some(true);
        } else {
          return _module.Option<bool>.create_Some(false);
        }
      } else {
        Dafny.ISequence<Dafny.Rune> _in0 = v;
        Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> _in1 = k;
    ...
    public static bool solveCircuit(Dafny.ISequence<_IcircuitCell> c, Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> k)
    {
      return __default.solveCircuit_k(c, k, (new BigInteger((c).Count)) - (BigInteger.One));
    }
    public static BigInteger getNumberFromPoz(_IcircuitCell e) {
      _IcircuitCell _source0 = e;
      {
        BigInteger _0_nr = _source0.dtor_i;
        return _0_nr;
    ...
    public static bool solveCircuit_k(Dafny.ISequence<_IcircuitCell> c, Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> k, BigInteger pos)
    {
      _IcircuitCell _source0 = (c).Select(pos);
      {
        if (_source0.is_AND) {
          return (__default.solveCircuit_k(c, k, __default.getNumberFromPoz((c).Select((pos) + (BigInteger.One))))) && (__default.solveCircuit_k(c, k, __default.getNumberFromPoz((c).Select((pos) + (new BigInteger(2))))));
        }
      }
      {
        if (_source0.is_OR) {
      ...
      {
        if (_source0.is_NOT) {
          return !(__default.solveCircuit_k(c, k, __default.getNumberFromPoz((c).Select((pos) + (BigInteger.One)))));
        }
      }
      {
        if (_source0.is_VARIABLE) {
          Dafny.ISequence<Dafny.Rune> _0_x = _source0.dtor_x;
          _IOption<bool> _source1 = __default.getValueFromCertificate(_0_x, k);
          {
            if (_source1.is_Some) {
              bool _1_st = _source1.dtor_t;
              return _1_st;
            }
          }
          {
      ...
      {
        BigInteger _2_nr = _source0.dtor_i;
        return __default.solveCircuit_k(c, k, _2_nr);
      }
    }
    public static _IConfiguration moveLeft(_IConfiguration con) {
      _IConfiguration _source0 = con;
      {
    ...
    public static _IConfiguration moveRight(_IConfiguration con) {
      _IConfiguration _source0 = con;
      {
        _IState _0_s = _source0.dtor_s;
        Func<BigInteger, _ISymbol> _1_tape = _source0.dtor_tape;
      ...
      _IConfiguration _source0 = config;
      {
        ...
        if (((delta).Keys).Contains(_module.Key.create(_0_s, Dafny.Helpers.Id<Func<BigInteger, _ISymbol>>(_1_tape)(_2_head)))) {
          _IAction _source1 = (Dafny.Map<_IKey, Dafny.ISequence<_IAction>>.Select(delta,_module.Key.create(_0_s, Dafny.Helpers.Id<Func<BigInteger, _ISymbol>>(_1_tape)(_2_head)))).Select(poz);
          {
            ...
            _IDirection _5_direction = _source1.dtor_direction;
            _IDirection _source2 = _5_direction;
            {
              if (_source2.is_Left) {
                return _module.Option<_IConfiguration>.create_Some(__default.moveLeft(_module.Configuration.create(_3_state, __default.changeSymbol(_1_tape, _2_head, _4_symbol), _2_head)));
              }
            }
            {
              return _module.Option<_IConfiguration>.create_Some(__default.moveRight(_module.Configuration.create(_3_state, __default.changeSymbol(_1_tape, _2_head, _4_symbol), _2_head)));
            }
          }
        } else {
          return _module.Option<_IConfiguration>.create_None();
        }
      }
    }
    public static Func<BigInteger, _ISymbol> changeSymbol(Func<BigInteger, _ISymbol> tape, BigInteger head, _ISymbol newSymbol)
    {
      return Dafny.Helpers.Id<Func<BigInteger, Func<BigInteger, _ISymbol>, _ISymbol, Func<BigInteger, _ISymbol>>>((_0_head, _1_tape, _2_newSymbol) => ((System.Func<BigInteger, _ISymbol>)((_3_i) => {
        return (((_3_i) != (_0_head)) ? (Dafny.Helpers.Id<Func<BigInteger, _ISymbol>>(_1_tape)(_3_i)) : (_2_newSymbol));
      })))(head, tape, newSymbol);
    }
    public static Func<BigInteger, _ISymbol> initialTape(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> input) {
      return Dafny.Helpers.Id<Func<Dafny.ISequence<Dafny.ISequence<Dafny.Rune>>, Func<BigInteger, _ISymbol>>>((_0_input) => ((System.Func<BigInteger, _ISymbol>)((_1_i) => {
    ...
    public static _IOption<_IConfiguration> runDTM_k(Dafny.IMap<_IKey,Dafny.ISequence<_IAction>> delta, _IConfiguration config, Dafny.ISet<_ISymbol> inputS, Dafny.ISet<_ISymbol> AddTapeS)
    {
      ...
      _IConfiguration _source0 = config;
      {
        _IState _0_q = _source0.dtor_s;
        Func<BigInteger, _ISymbol> _1_tape = _source0.dtor_tape;
        BigInteger _2_head = _source0.dtor_head;
        _IState _source1 = _0_q;
        {
          if (_source1.is_FinalState) {
            _IConclusion _3_finale = _source1.dtor_conclusion;
            con = _module.Option<_IConfiguration>.create_Some(config);
            return con;
            goto after_match1;
          }
        }
        {
          Dafny.ISequence<Dafny.Rune> _4_q0 = _source1.dtor_s;
          {
            ...
            {
              if (_source2.is_Some) {
                _IConfiguration _6_c = _source2.dtor_t;
                {
            ...
            {
              con = _module.Option<_IConfiguration>.create_None();
              return con;
            }
          after_match2: ;
          }
  ...
  public interface _IcircuitCell {
    bool is_AND { get; }
    bool is_OR { get; }
    bool is_NOT { get; }
    bool is_VARIABLE { get; }
    bool is_POZ { get; }
    Dafny.ISequence<Dafny.Rune> dtor_x { get; }
    BigInteger dtor_i { get; }
    _IcircuitCell DowncastClone();
  }
  public abstract class circuitCell : _IcircuitCell {
    public circuitCell() {
    }
    private static readonly _IcircuitCell theDefault = create_AND();
    public static _IcircuitCell Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_IcircuitCell> _TYPE = new Dafny.TypeDescriptor<_IcircuitCell>(circuitCell.Default());
    public static Dafny.TypeDescriptor<_IcircuitCell> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IcircuitCell create_AND() {
      return new circuitCell_AND();
    }
    public static _IcircuitCell create_OR() {
      return new circuitCell_OR();
    }
    public static _IcircuitCell create_NOT() {
      return new circuitCell_NOT();
    }
    public static _IcircuitCell create_VARIABLE(Dafny.ISequence<Dafny.Rune> x) {
      return new circuitCell_VARIABLE(x);
    }
    public static _IcircuitCell create_POZ(BigInteger i) {
      return new circuitCell_POZ(i);
    }
    public bool is_AND { get { return this is circuitCell_AND; } }
    public bool is_OR { get { return this is circuitCell_OR; } }
    public bool is_NOT { get { return this is circuitCell_NOT; } }
    public bool is_VARIABLE { get { return this is circuitCell_VARIABLE; } }
    public bool is_POZ { get { return this is circuitCell_POZ; } }
    public Dafny.ISequence<Dafny.Rune> dtor_x {
      get {
        var d = this;
        return ((circuitCell_VARIABLE)d)._x;
      }
    }
    public BigInteger dtor_i {
      get {
        var d = this;
        return ((circuitCell_POZ)d)._i;
      }
    }
    public abstract _IcircuitCell DowncastClone();
  }
  public class circuitCell_AND : circuitCell {
    public circuitCell_AND() : base() {
    }
    public override _IcircuitCell DowncastClone() {
      if (this is _IcircuitCell dt) { return dt; }
      return new circuitCell_AND();
    }
    public override bool Equals(object other) {
      var oth = other as circuitCell_AND;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "circuitCell.AND";
      return s;
    }
  }
  public class circuitCell_OR : circuitCell {
    public circuitCell_OR() : base() {
    }
    public override _IcircuitCell DowncastClone() {
      if (this is _IcircuitCell dt) { return dt; }
      return new circuitCell_OR();
    ...
    public override bool Equals(object other) {
      var oth = other as circuitCell_OR;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "circuitCell.OR";
      return s;
    }
  }
  public class circuitCell_NOT : circuitCell {
    public circuitCell_NOT() : base() {
    }
    public override _IcircuitCell DowncastClone() {
      if (this is _IcircuitCell dt) { return dt; }
      return new circuitCell_NOT();
    ...
    public override bool Equals(object other) {
      var oth = other as circuitCell_NOT;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "circuitCell.NOT";
      return s;
    }
  }
  public class circuitCell_VARIABLE : circuitCell {
    public readonly Dafny.ISequence<Dafny.Rune> _x;
    public circuitCell_VARIABLE(Dafny.ISequence<Dafny.Rune> x) : base() {
      this._x = x;
    }
    public override _IcircuitCell DowncastClone() {
      if (this is _IcircuitCell dt) { return dt; }
      return new circuitCell_VARIABLE(_x);
    }
    public override bool Equals(object other) {
      var oth = other as circuitCell_VARIABLE;
      return oth != null && object.Equals(this._x, oth._x);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 3;
    ...
    public override string ToString() {
      string s = "circuitCell.VARIABLE";
      s += "(";
      s += this._x.ToVerbatimString(true);
      s += ")";
      return s;
    }
  }
  public class circuitCell_POZ : circuitCell {
    public readonly BigInteger _i;
    public circuitCell_POZ(BigInteger i) : base() {
      this._i = i;
    }
    public override _IcircuitCell DowncastClone() {
      if (this is _IcircuitCell dt) { return dt; }
      return new circuitCell_POZ(_i);
    }
    public override bool Equals(object other) {
      var oth = other as circuitCell_POZ;
  ...
  public partial class stringNat {
    ...
    private static readonly Dafny.TypeDescriptor<Dafny.ISequence<Dafny.Rune>> _TYPE = new Dafny.TypeDescriptor<Dafny.ISequence<Dafny.Rune>>(stringNat.Default());
    public static Dafny.TypeDescriptor<Dafny.ISequence<Dafny.Rune>> _TypeDescriptor() {
      return _TYPE;
    }
    public static bool _Is(Dafny.ISequence<Dafny.Rune> __source) {
      Dafny.ISequence<Dafny.Rune> _0_s = __source;
      return (((new BigInteger((_0_s).Count)).Sign == 1) && (!((new BigInteger((_0_s).Count)) > (BigInteger.One)) || (((_0_s).Select(BigInteger.Zero)) != (new Dafny.Rune('0'))))) && (Dafny.Helpers.Id<Func<Dafny.ISequence<Dafny.Rune>, bool>>((_1_s) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_1_s).Count)), true, (((_forall_var_0) => {
        BigInteger _2_i = (BigInteger)_forall_var_0;
        return !(((_2_i).Sign != -1) && ((_2_i) < (new BigInteger((_1_s).Count)))) || ((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("0123456789")).Contains((_1_s).Select(_2_i)));
      }))))(_0_s));
    }
  }

  public interface _ISymbol {
    bool is_NonBlankSymbol { get; }
    bool is_Blank { get; }
  ...
  public abstract class Symbol : _ISymbol {
    ...
    private static readonly _ISymbol theDefault = create_NonBlankSymbol(Dafny.Sequence<Dafny.Rune>.Empty);
    public static _ISymbol Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_ISymbol> _TYPE = new Dafny.TypeDescriptor<_ISymbol>(Symbol.Default());
    public static Dafny.TypeDescriptor<_ISymbol> _TypeDescriptor() {
      return _TYPE;
    }
    public static _ISymbol create_NonBlankSymbol(Dafny.ISequence<Dafny.Rune> s) {
      return new Symbol_NonBlankSymbol(s);
    }
    public static _ISymbol create_Blank() {
      return new Symbol_Blank();
    }
    public bool is_NonBlankSymbol { get { return this is Symbol_NonBlankSymbol; } }
    public bool is_Blank { get { return this is Symbol_Blank; } }
    public Dafny.ISequence<Dafny.Rune> dtor_s {
      get {
        var d = this;
        return ((Symbol_NonBlankSymbol)d)._s;
      }
    }
    public abstract _ISymbol DowncastClone();
  }
  public class Symbol_NonBlankSymbol : Symbol {
    public readonly Dafny.ISequence<Dafny.Rune> _s;
    public Symbol_NonBlankSymbol(Dafny.ISequence<Dafny.Rune> s) : base() {
      this._s = s;
    }
    public override _ISymbol DowncastClone() {
      if (this is _ISymbol dt) { return dt; }
      return new Symbol_NonBlankSymbol(_s);
    }
    public override bool Equals(object other) {
      var oth = other as Symbol_NonBlankSymbol;
    ...
    public override string ToString() {
      string ss = "Symbol.NonBlankSymbol";
      ss += "(";
  ...
  public class Symbol_Blank : Symbol {
    public Symbol_Blank() : base() {
    }
    public override _ISymbol DowncastClone() {
      if (this is _ISymbol dt) { return dt; }
      return new Symbol_Blank();
    ...
    public override string ToString() {
      string s = "Symbol.Blank";
      return s;
    }
  }

  public interface _IConclusion {
    bool is_Accept { get; }
    bool is_Reject { get; }
    _IConclusion DowncastClone();
  }
  public abstract class Conclusion : _IConclusion {
    public Conclusion() {
    }
    private static readonly _IConclusion theDefault = create_Accept();
    public static _IConclusion Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_IConclusion> _TYPE = new Dafny.TypeDescriptor<_IConclusion>(Conclusion.Default());
    public static Dafny.TypeDescriptor<_IConclusion> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IConclusion create_Accept() {
      return new Conclusion_Accept();
    }
    public static _IConclusion create_Reject() {
      return new Conclusion_Reject();
    }
    public bool is_Accept { get { return this is Conclusion_Accept; } }
    public bool is_Reject { get { return this is Conclusion_Reject; } }
    public static System.Collections.Generic.IEnumerable<_IConclusion> AllSingletonConstructors {
      get {
        yield return Conclusion.create_Accept();
        yield return Conclusion.create_Reject();
      }
    }
    public abstract _IConclusion DowncastClone();
  }
  public class Conclusion_Accept : Conclusion {
    public Conclusion_Accept() : base() {
    }
    public override _IConclusion DowncastClone() {
      if (this is _IConclusion dt) { return dt; }
      return new Conclusion_Accept();
    }
    public override bool Equals(object other) {
      var oth = other as Conclusion_Accept;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "Conclusion.Accept";
      return s;
    }
  }
  public class Conclusion_Reject : Conclusion {
    public Conclusion_Reject() : base() {
    }
    public override _IConclusion DowncastClone() {
      if (this is _IConclusion dt) { return dt; }
      return new Conclusion_Reject();
    ...
    public override bool Equals(object other) {
      var oth = other as Conclusion_Reject;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "Conclusion.Reject";
      return s;
    }
  }

  public interface _IState {
    bool is_NormalState { get; }
    bool is_FinalState { get; }
    Dafny.ISequence<Dafny.Rune> dtor_s { get; }
    _IConclusion dtor_conclusion { get; }
    _IState DowncastClone();
  }
  public abstract class State : _IState {
    public State() {
    }
    private static readonly _IState theDefault = create_NormalState(Dafny.Sequence<Dafny.Rune>.Empty);
    ...
    public static _IState create_NormalState(Dafny.ISequence<Dafny.Rune> s) {
      return new State_NormalState(s);
    }
    public static _IState create_FinalState(Dafny.ISequence<Dafny.Rune> s, _IConclusion conclusion) {
      return new State_FinalState(s, conclusion);
    }
    public bool is_NormalState { get { return this is State_NormalState; } }
    public bool is_FinalState { get { return this is State_FinalState; } }
    public Dafny.ISequence<Dafny.Rune> dtor_s {
      get {
        var d = this;
        if (d is State_NormalState) { return ((State_NormalState)d)._s; }
        return ((State_FinalState)d)._s;
      }
    }
    public _IConclusion dtor_conclusion {
      get {
        var d = this;
        return ((State_FinalState)d)._conclusion;
      }
    }
    public abstract _IState DowncastClone();
  }
  public class State_NormalState : State {
    public readonly Dafny.ISequence<Dafny.Rune> _s;
    public State_NormalState(Dafny.ISequence<Dafny.Rune> s) : base() {
      this._s = s;
    }
    public override _IState DowncastClone() {
      if (this is _IState dt) { return dt; }
      return new State_NormalState(_s);
    }
    public override bool Equals(object other) {
      var oth = other as State_NormalState;
      return oth != null && object.Equals(this._s, oth._s);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
  ...
  public class State_FinalState : State {
    public readonly Dafny.ISequence<Dafny.Rune> _s;
    public readonly _IConclusion _conclusion;
    public State_FinalState(Dafny.ISequence<Dafny.Rune> s, _IConclusion conclusion) : base() {
      this._s = s;
      this._conclusion = conclusion;
    }
    public override _IState DowncastClone() {
      if (this is _IState dt) { return dt; }
      return new State_FinalState(_s, _conclusion);
    }
    public override bool Equals(object other) {
      var oth = other as State_FinalState;
      return oth != null && object.Equals(this._s, oth._s) && object.Equals(this._conclusion, oth._conclusion);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 1;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._s));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._conclusion));
      return (int) hash;
    }
    public override string ToString() {
      string ss = "State.FinalState";
      ss += "(";
      ss += this._s.ToVerbatimString(true);
      ss += ", ";
      ss += Dafny.Helpers.ToString(this._conclusion);
      ss += ")";
      return ss;
    }
  }

  public interface _IConfiguration {
    bool is_Configuration { get; }
    _IState dtor_s { get; }
    Func<BigInteger, _ISymbol> dtor_tape { get; }
    BigInteger dtor_head { get; }
    _IConfiguration DowncastClone();
  }
  public class Configuration : _IConfiguration {
    public readonly _IState _s;
    public readonly Func<BigInteger, _ISymbol> _tape;
    public readonly BigInteger _head;
    public Configuration(_IState s, Func<BigInteger, _ISymbol> tape, BigInteger head) {
      this._s = s;
      this._tape = tape;
      this._head = head;
    }
    public _IConfiguration DowncastClone() {
      if (this is _IConfiguration dt) { return dt; }
      return new Configuration(_s, _tape, _head);
    }
    public override bool Equals(object other) {
      var oth = other as Configuration;
      return oth != null && object.Equals(this._s, oth._s) && object.Equals(this._tape, oth._tape) && this._head == oth._head;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._s));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._tape));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._head));
      return (int) hash;
    }
    public override string ToString() {
      string ss = "Configuration.Configuration";
      ss += "(";
      ss += Dafny.Helpers.ToString(this._s);
      ss += ", ";
      ss += Dafny.Helpers.ToString(this._tape);
      ss += ", ";
      ss += Dafny.Helpers.ToString(this._head);
      ss += ")";
      return ss;
    }
    private static readonly _IConfiguration theDefault = create(State.Default(), ((BigInteger x0) => Symbol.Default()), BigInteger.Zero);
    public static _IConfiguration Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_IConfiguration> _TYPE = new Dafny.TypeDescriptor<_IConfiguration>(Configuration.Default());
    public static Dafny.TypeDescriptor<_IConfiguration> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IConfiguration create(_IState s, Func<BigInteger, _ISymbol> tape, BigInteger head) {
      return new Configuration(s, tape, head);
    }
    public static _IConfiguration create_Configuration(_IState s, Func<BigInteger, _ISymbol> tape, BigInteger head) {
      return create(s, tape, head);
    }
    public bool is_Configuration { get { return true; } }
    public _IState dtor_s {
      get {
        return this._s;
      }
    }
    public Func<BigInteger, _ISymbol> dtor_tape {
      get {
        return this._tape;
      }
    }
  ...
  public interface _IKey {
    bool is_Key { get; }
    _IState dtor_state { get; }
    _ISymbol dtor_symbol { get; }
    _IKey DowncastClone();
  }
  public class Key : _IKey {
    public readonly _IState _state;
    public readonly _ISymbol _symbol;
    public Key(_IState state, _ISymbol symbol) {
      this._state = state;
      this._symbol = symbol;
    }
    public _IKey DowncastClone() {
      if (this is _IKey dt) { return dt; }
      return new Key(_state, _symbol);
    }
    public override bool Equals(object other) {
      var oth = other as Key;
      return oth != null && object.Equals(this._state, oth._state) && object.Equals(this._symbol, oth._symbol);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._state));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._symbol));
      return (int) hash;
    }
    public override string ToString() {
      string s = "Key.Key";
      ...
      s += ", ";
      s += Dafny.Helpers.ToString(this._symbol);
      s += ")";
      return s;
    }
    private static readonly _IKey theDefault = create(State.Default(), Symbol.Default());
    public static _IKey Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_IKey> _TYPE = new Dafny.TypeDescriptor<_IKey>(Key.Default());
    public static Dafny.TypeDescriptor<_IKey> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IKey create(_IState state, _ISymbol symbol) {
      return new Key(state, symbol);
    }
    public static _IKey create_Key(_IState state, _ISymbol symbol) {
      return create(state, symbol);
    }
    public bool is_Key { get { return true; } }
    public _IState dtor_state {
      get {
        return this._state;
      }
    }
    public _ISymbol dtor_symbol {
      get {
        return this._symbol;
      }
    }
  ...
  public interface _IDirection {
    bool is_Left { get; }
    bool is_Right { get; }
    _IDirection DowncastClone();
  }
  public abstract class Direction : _IDirection {
    public Direction() {
    }
    private static readonly _IDirection theDefault = create_Left();
    public static _IDirection Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_IDirection> _TYPE = new Dafny.TypeDescriptor<_IDirection>(Direction.Default());
    public static Dafny.TypeDescriptor<_IDirection> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IDirection create_Left() {
      return new Direction_Left();
    }
    public static _IDirection create_Right() {
      return new Direction_Right();
    }
    public bool is_Left { get { return this is Direction_Left; } }
    public bool is_Right { get { return this is Direction_Right; } }
    public static System.Collections.Generic.IEnumerable<_IDirection> AllSingletonConstructors {
      get {
        yield return Direction.create_Left();
        yield return Direction.create_Right();
      }
    }
    public abstract _IDirection DowncastClone();
  }
  public class Direction_Left : Direction {
    public Direction_Left() : base() {
    }
    public override _IDirection DowncastClone() {
      if (this is _IDirection dt) { return dt; }
      return new Direction_Left();
    }
    public override bool Equals(object other) {
  ...
  public class Direction_Right : Direction {
    public Direction_Right() : base() {
    }
    public override _IDirection DowncastClone() {
      if (this is _IDirection dt) { return dt; }
      return new Direction_Right();
  ...
  public interface _IAction {
    ...
    _IState dtor_state { get; }
    _ISymbol dtor_symbol { get; }
    _IDirection dtor_direction { get; }
    _IAction DowncastClone();
  }
  public class Action : _IAction {
    public readonly _IState _state;
    public readonly _ISymbol _symbol;
    public readonly _IDirection _direction;
    public Action(_IState state, _ISymbol symbol, _IDirection direction) {
      this._state = state;
      this._symbol = symbol;
      this._direction = direction;
    }
    public _IAction DowncastClone() {
      if (this is _IAction dt) { return dt; }
      return new Action(_state, _symbol, _direction);
    }
    public override bool Equals(object other) {
      var oth = other as Action;
      return oth != null && object.Equals(this._state, oth._state) && object.Equals(this._symbol, oth._symbol) && object.Equals(this._direction, oth._direction);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._state));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._symbol));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._direction));
      return (int) hash;
    }
    public override string ToString() {
      ...
      s += Dafny.Helpers.ToString(this._symbol);
      s += ", ";
      s += Dafny.Helpers.ToString(this._direction);
      s += ")";
      return s;
    }
    private static readonly _IAction theDefault = create(State.Default(), Symbol.Default(), Direction.Default());
    public static _IAction Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_IAction> _TYPE = new Dafny.TypeDescriptor<_IAction>(Action.Default());
    public static Dafny.TypeDescriptor<_IAction> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IAction create(_IState state, _ISymbol symbol, _IDirection direction) {
      return new Action(state, symbol, direction);
    }
    public static _IAction create_Action(_IState state, _ISymbol symbol, _IDirection direction) {
      return create(state, symbol, direction);
    }
    public bool is_Action { get { return true; } }
    public _IState dtor_state {
      get {
        return this._state;
      }
    ...
    public _IDirection dtor_direction {
      get {
  ...
  public abstract class Option<T> : _IOption<T> {
    ...
    public static _IOption<T> create_Some(T t) {
      return new Option_Some<T>(t);
    }
